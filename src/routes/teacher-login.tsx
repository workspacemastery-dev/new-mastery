...
import { TeacherLoginForm } from "@/components/auth/AcademyLoginForms";
...
export const Route = createFileRoute("/teacher-login")({
  ...
  head: () => ({ meta: [{ title: "دخول المعلم — EduVerse" }] }),
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative max-w-md mx-auto px-6 py-16">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold mb-2">دخول المعلم</h1>
        ...
        <div className="bg-card-premium border border-border/60 rounded-2xl p-6 shadow-elevated">
          {academy ? <TeacherLoginForm academyId={academy.id} /> : <MissingAcademy />}
        </div>
      </div>
    </div>
  );
...
function MissingAcademy() {
  return (
    <div className="text-center space-y-4">
      <p className="text-sm text-muted-foreground">لا يمكن فتح دخول المعلم بدون تحديد الأكاديمية.</p>
...
function LoginError({ message }: { message: string }) {
  ...
  return (
    <div className="min-h-screen grid place-items-center px-6 text-center">
      <div>
        <h1 className="text-2xl font-bold mb-2">تعذّر فتح دخول المعلم</h1>
