...
export const checkSuperAdminBootstrap = createServerFn({ method: "GET" }).handler(
  async () => ({ needsBootstrap: !(await hasAnySuperAdmin()) })
);
...
export const setTeacherDisabled = createServerFn({ method: "POST" })
  .inputValidator(
    (input: { accessToken: string; userId: string; disabled: boolean }) => {
...
export const resetUserPassword = createServerFn({ method: "POST" })
  .inputValidator(
    (input: { accessToken: string; email: string; newPassword: string }) => {
